@if ($post->post_type == 'video')
    @if ($post->video_id != null)
        ...
        @if (isFileExist(@$post->videoThumbnail, @$post->videoThumbnail->big_image_two))
            <video id='player' autoplay controls poster="{{ basePath($post->videoThumbnail) }}/{{ $post->videoThumbnail->big_image_two }} ">
            @else
                <video id='player' autoplay controls poster="{{ static_asset('default-image/default-730x400.png') }}">
        @endif
        {{-- $this->video->get_all_videos($id); --}}
        @if ($post->video->v_144p == null and $post->video->v_240p == null and $post->video->v_360p == null and $post->video->v_480p == null and $post->video->v_720p == null and $post->video->v_1080p == null)
    ...
    @else
        @if ($post->video_url_type == 'youtube_url')
            <div id="player" autoplay data-plyr-provider="youtube" data-plyr-embed-id="{{ get_id_youtube($post->video_url) }}"></div>
        @elseif($post->video_url_type == 'mp4_url')
            <video id="player" autoplay playsinline controls data-poster="{{ basePath(@$post->image) }}/{{ @$post->image->big_image_two }}">
