                ...
                <div class="col-sm-12">
                    <div class="filters text-center">
                        <div class="button-group filter-button-group">
                            <button class="btn btn-primary filter active" data-filter="*">همه</button>
                            @isset($tabs)
                        ...
                        @foreach($albumImages as $albumImage)
                            ...
                            <a href="{{ isFileExist(@$albumImage, $result = @$albumImage->original_image) ? basePath($albumImage).'/'.@$result : static_asset('default-image/default-1080x1000.png') }}" data-title="{{ $albumImage->title }}" data-toggle="lightbox" data-gallery="example-gallery">
                                <div class="grid-item {{ str_replace(' ', '-', $albumImage->tab) }}" data-category="{{ str_replace(' ', '-', $albumImage->tab) }}">
                                    <img src="{{ isFileExist(@$albumImage, $result = @$albumImage->original_image) ? basePath($albumImage).'/'.@$result : static_asset('default-image/default-1080x1000.png') }}" alt="{{ $albumImage->title }}" class="lightboxed" rel="img-{{$albumImage->title}}" data-caption="{{$albumImage->title}}">
                                </div>
                            </a>
                        @endforeach
                    </div>
