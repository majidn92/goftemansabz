...
<header class="sg-header">
    <div class="sg-topbar">
        <div id="sg-top" style="margin-left:50px;float:left">
            <div class="d-md-flex" style="justify-content: flex-end;">
                <div style="padding-top: 5px">
                    <ul class="global-list">
                        <li><i class="fa fa-calendar ml-2" aria-hidden="true"></i>{{ miladi_to_jalali(date('Y-m-d H:i:s')) }}</li>
                    </ul>
                </div>
                <div>
                    <div class="d-flex ">
        ...
        <div id="sg-bottom" style="margin-right:92px">
            {{-- @include('site.partials.breaking_news') --}}
            @php
            $breaking_news = Modules\Post\Entities\Post::where('visibility',1)->where('status',1)->where('breaking', 1)->orderBy('breaking_order', 'asc')->orderBy('created_at', 'desc')->where('section_id',1)->get();
            @endphp
            <ul class="news-ticker">
                @foreach ($breaking_news as $item)
                <li style="margin-left: unset"><a href="{{ route('article.detail', [$item->slug]) }}">{{ $item->title }}</a></li>
                @endforeach
            </ul>
        </div>
    </div>
        ...
        <nav class="navbar navbar-expand-lg">
            <div class="container" style="max-width: 90%">
                <div class="menu-content">
                    ...
                    <div class="collapse navbar-collapse justify-content-start" id="navbarNav">
                        <ul class="navbar-nav">
                            ...
                            <li class="nav-item sg-dropdown">
                                <a href="{{ menuUrl($mainMenu) }}" target="{{ $mainMenu->new_teb == 1 ? '_blank' : '' }}">{{ $mainMenu->label == 'gallery' ? __('gallery') : $mainMenu->label }} @if (!blank($mainMenu->children))
                                    <span><i class="fa fa-angle-down" aria-hidden="true"></i></span>
                                    @endif </a>
                                <ul class="sg-dropdown-menu">
                                    @foreach ($mainMenu->children as $child)
                                    <li class=""><a href="{{ menuUrl($child) }}" target="{{ $child->new_teb == 1 ? '_blank' : '' }}">{{ $child->label == 'gallery' ? __('gallery') : $child->label }} @if (!blank($child->children))
                    ...
                    <button id="search-btn" style="margin-left: -20px;border :unset;background-color: white;float: left"><i class="fa fa-search"></i><span class="d-none">{{ __('search') }}</span></button>
                </div>
            </div>
        </nav>
    </div>
