...
class SettingController extends Controller
{
    ...
    {
        $activeLang     = Language::where('status', 'active')->orderBy('name', 'ASC')->get();
        $sections = \DB::table('sections')->orderby('rank', 'asc')->get();
        // dd($sections);
    ...
    public function section_store(Request $request)
    {
        ...
        $section->slider = isset($request->slider) && isset($request->show) ? 1 : 0;
        $section->last_post = isset($request->last_post) && isset($request->show) ? 1 : 0;
        $section->video = isset($request->video) && isset($request->show) ? 1 : 0;
