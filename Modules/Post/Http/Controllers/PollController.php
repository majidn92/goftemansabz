...
class PollController extends Controller
{
    ...
    {
        $messages = [
            'option.min' => 'حداقل دو گزینه برای پاسخ وارد نمائید',
            'end_date.after_or_equal' => 'تاریخ پایان نظرسنجی باید بعد از تاریخ شروع نظرسنجی باشد',
        ];

        $data = $request->except('_token');
        $data['start_date'] = jalali_to_miladi($request->start_date);
        $data['end_date'] = jalali_to_miladi($request->end_date);
        Validator::make($data, [
        ...
        try {
            ...
            $poll->start_date       = date('Y-m-d H:i:s', strtotime(jalali_to_miladi($request->start_date)));
            $poll->end_date         = date('Y-m-d H:i:s', strtotime(jalali_to_miladi($request->start_date)));
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $poll->auth_required    = $request->auth_required;
            $poll->status           = $request->status;
            $poll->start_date       = date(jalali_to_miladi($request->start_date));
            $poll->end_date         = date(jalali_to_miladi($request->end_date));
